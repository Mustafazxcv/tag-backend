...
const getCurrentScooter = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(201).json({
        error: true,
        message: 'Şu anda aktif olarak bir scooter kullanmıyorsunuz.',
        data: null
      });
    }
    ...
    res.status(500).json({
      error: true,
      message: 'Aktif scooter bilgisi alınırken bir hata oluştu.',
      data: null
    });
  }
};
  ...
  const endScooterRental = async (req, res) => {
    const userId = req.user.id; 
    const { scooter_id, latitude, longitude, battery_level } = req.body; 
